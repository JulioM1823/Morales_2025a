...
_XCORRJ_ENGINE_DEFAULT = 'chunked'
_XCORRJ_PARALLEL_DEFAULT = True
_XCORRJ_CHUNK_CENTERS_DEFAULT = 'auto'
_XCORRJ_CHUNK_MEMORY_MB_DEFAULT = 128.0
_XCORR_GEOMETRY_DEFAULT = 'annulus'
_XCORR_GEOMETRIES = ('annulus', 'east', 'west', 'north', 'south')
_XCORR_DIRECTIONAL_GEOMETRIES = ('east', 'west', 'north', 'south')
...
class TimeDistanceResult:
    xcorr: np.ndarray
    phase: np.ndarray
    skip_distances: np.ndarray


def lag_axis_minutes(nt: int, dt: float) -> np.ndarray:

    '''Return the fftshifted lag axis in minutes.'''

    return (np.arange(int(nt), dtype = float) - int(nt)//2)*float(dt)/60.0


def peak_lag_from_xcorr(xcorr: np.ndarray) -> int:

    '''Return the lag sample at the maximum of an fftshifted correlation.'''

    values = np.asarray(xcorr)

    return int(np.nanargmax(values) - values.size//2)


def synthetic_delay_cube(
    *,
    nt: int,
    ny: int,
    nx: int,
    ...
    '''Build a deterministic cube pair with the second cube delayed in time.'''

    rng = np.random.default_rng(seed)
    v1 = rng.standard_normal((int(nt), int(ny), int(nx)), dtype = np.float32)
    v2 = np.roll(v1, int(lag_samples), axis = 0)
    if float(noise_std) > 0.0:
        v2 = v2 + rng.normal(0.0, float(noise_std), size = v2.shape).astype(np.float32)

    return v1.astype(np.float32, copy = False), v2.astype(np.float32, copy = False)


def _invalid_edge(edge: np.ndarray) -> bool:

    return bool(np.all(~np.isfinite(edge) | (edge == 0.0)))


def trim_spatial_zero_nan_borders(v1: np.ndarray, v2: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:

    '''Trim shared spatial borders that are entirely zero or NaN in either cube.'''

    cube_1 = np.asarray(v1)
    cube_2 = np.asarray(v2)
    if cube_1.shape != cube_2.shape:
        raise ValueError('v1 and v2 must have the same shape.')
    if cube_1.ndim != 3:
        raise ValueError('v1 and v2 must have shape [t, y, x].')
    ...
    changed = True
    while changed and y1 - y0 > 1 and x1 - x0 > 1:
        changed = False
        if _invalid_edge(cube_1[:, y0, x0:x1]) or _invalid_edge(cube_2[:, y0, x0:x1]):
            y0 += 1
            changed = True
        if y1 - y0 > 1 and (_invalid_edge(cube_1[:, y1 - 1, x0:x1]) or _invalid_edge(cube_2[:, y1 - 1, x0:x1])):
            y1 -= 1
            changed = True
        if x1 - x0 > 1 and (_invalid_edge(cube_1[:, y0:y1, x0]) or _invalid_edge(cube_2[:, y0:y1, x0])):
            x0 += 1
            changed = True
        if x1 - x0 > 1 and (_invalid_edge(cube_1[:, y0:y1, x1 - 1]) or _invalid_edge(cube_2[:, y0:y1, x1 - 1])):
            x1 -= 1
            changed = True

    return cube_1[:, y0:y1, x0:x1], cube_2[:, y0:y1, x0:x1]


def _direction_offsets(skip_pixels: int) -> Tuple[Tuple[int, int], ...]:
...
def _overlap_for_offset(v1: np.ndarray, v2: np.ndarray, dy: int, dx: int) -> Tuple[np.ndarray, np.ndarray]:

    y1_start = max(0, -dy)
    y1_stop = min(v1.shape[1], v1.shape[1] - dy)
    x1_start = max(0, -dx)
    x1_stop = min(v1.shape[2], v1.shape[2] - dx)
    ...
    return (
        v1[:, y1_start:y1_stop, x1_start:x1_stop],
        v2[:, y2_start:y2_stop, x2_start:x2_stop],
    )


def _mean_cross_correlation(v1: np.ndarray, v2: np.ndarray, *, normalize: bool) -> np.ndarray:

    nt = int(v1.shape[0])
    a = np.asarray(v1, dtype = np.float64).reshape(nt, -1)
    b = np.asarray(v2, dtype = np.float64).reshape(nt, -1)
    valid = np.all(np.isfinite(a), axis = 0) & np.all(np.isfinite(b), axis = 0)
    if not np.any(valid):
        return np.full(nt, np.nan, dtype = np.float64)
    ...
    if normalize:
        ...
        b_std = np.std(b, axis = 0, keepdims = True)
        a_std[a_std == 0.0] = 1.0
        b_std[b_std == 0.0] = 1.0
        a = a/a_std
        b = b/b_std

    cross = np.fft.ifft(np.conj(np.fft.fft(a, axis = 0))*np.fft.fft(b, axis = 0), axis = 0).real

    return np.fft.fftshift(np.mean(cross, axis = 1))


def _compute_time_distance_result(
    v1: np.ndarray,
    v2: np.ndarray,
    *,
    skip_distances: Iterable[float],
    width: int,
    normalize: bool,
    trim_zero_nan_borders: bool,
) -> TimeDistanceResult:
    ...
    cube_1 = np.asarray(v1)
    cube_2 = np.asarray(v2)
    if trim_zero_nan_borders:
        cube_1, cube_2 = trim_spatial_zero_nan_borders(cube_1, cube_2)

    skip_array = np.asarray(list(skip_distances), dtype = np.float64)
    nt = int(cube_1.shape[0])
    xcorr = np.zeros((len(DIRECTION_ORDER), skip_array.size, nt), dtype = np.float64)

    for skip_index, skip_distance in enumerate(skip_array):
        skip_pixels = int(round(float(skip_distance)))
        for direction_index, (dy, dx) in enumerate(_direction_offsets(skip_pixels)):
            lower, upper = _overlap_for_offset(cube_1, cube_2, dy, dx)
            xcorr[direction_index, skip_index, :] = _mean_cross_correlation(
                lower,
                upper,
                normalize = normalize,
            )

    phase = np.angle(np.fft.fftshift(np.fft.fft(xcorr, axis = -1), axes = -1))

    return TimeDistanceResult(xcorr = xcorr, phase = phase, skip_distances = skip_array)


def time_distance_baseline(
    v1: np.ndarray,
    v2: np.ndarray,
    *,
    skip_distances: Iterable[float],
    ...
    n_jobs: int = 1,
    normalize: bool = True,
    legacy_axis_swap: bool = False,
    trim_zero_nan_borders: bool = True,
) -> TimeDistanceResult:

    del n_jobs, legacy_axis_swap

    return _compute_time_distance_result(
        v1,
        v2,
        skip_distances = skip_distances,
        width = width,
        normalize = normalize,
        trim_zero_nan_borders = trim_zero_nan_borders,
    )


def time_distance_fast(
    v1: np.ndarray,
    v2: np.ndarray,
    *,
    skip_distances: Iterable[float],
    ...
    normalize: bool = True,
    legacy_axis_swap: bool = False,
    phase_average_mode: str = 'legacy',
    time_chunk: Optional[int] = None,
    trim_zero_nan_borders: bool = True,
) -> TimeDistanceResult:

    del legacy_axis_swap, phase_average_mode, time_chunk
...
def _deep_merge(defaults, overrides):
    ...
    Purpose
    -------
    Recursively merge a nested override dictionary into a default dictionary.

    Inputs
    ------
    defaults: object
        Default scalar value or mapping.

    overrides: object
        User-supplied scalar value or mapping.
    ...
    # Replace scalars and lists outright instead of attempting itemwise merges.
    if not isinstance(defaults, dict) or not isinstance(overrides, dict):
        return copy.deepcopy(overrides)

    # Merge nested dictionaries recursively so callers can override one branch.
...
def _strip_dispersion_curve_style_keys(value):

    '''
    Purpose
    -------
    Remove presentation-layer keys from a dispersion-curve configuration tree.

    Inputs
    ------
    value: object
        Candidate dispersion-curve configuration value.

    Outputs
    -------
    stripped_value: object
        Deep-copied configuration containing only computation/model settings.

    Author(s)
    ---------
    Julio M. Morales, April 27th, 2026.
    '''

    if isinstance(value, list):
        return [_strip_dispersion_curve_style_keys(item) for item in value]

    if isinstance(value, dict):
        return {
            key: _strip_dispersion_curve_style_keys(item)
            for key, item in value.items()
            if key not in _DISPERSION_CURVE_STYLE_KEYS
        }
...
def _resolve_module_dir(reference_dir = None):

    '''
    Purpose
    -------
    Resolve the module directory used by the time-distance helper functions.

    Inputs
    ------
    reference_dir: str or pathlib.Path, optional
        Explicit module directory override.

    Outputs
    -------
    module_dir: pathlib.Path
        Resolved `Code/Time-Distance` directory.
    ...
    # Prefer the explicit reference directory when one was supplied.
    if reference_dir not in ['', None]:
        return Path(reference_dir).expanduser().resolve()

    return Path(__file__).resolve().parent


def _build_default_dispersion_curve_config(reference_dir = None):

    '''
    Purpose
    -------
    Build the default dispersion-curve model configuration used by the pipeline.

    Inputs
    ------
    reference_dir: str or pathlib.Path, optional
        Directory used to locate the legacy reference atmosphere.

    Outputs
    -------
    config: dict
        Default dispersion-curve configuration dictionary.
    ...
    module_dir = _resolve_module_dir(reference_dir)

    return {
        'mode': 'simulation_based',
        'simulation_based': {
            'reference_model_file': str(module_dir / 'Oana_codes' / 'CSM_A.dat'),
            'height_index': 1005,
            'gravity_cgs': 27400.0,
            'gravity_km_s2': 0.274,
            'curves': {
...
def _normalize_dispersion_curve_mode(mode):

    '''
    Purpose
    -------
    Normalize the configured dispersion-curve mode.

    Inputs
    ------
    mode: object
        Raw dispersion-curve mode.
    ...
    # Default to the legacy overlay mode when the caller leaves the value blank.
    if mode in ['', None]:
        return 'simulation_based'

    normalized_mode = str(mode).strip().lower()
    if normalized_mode not in ['simulation_based', 'theory_based']:
        raise ValueError(
            "dispersion_curves['mode'] must be either 'simulation_based' or 'theory_based'."
        )
...
def _normalize_dispersion_curve_toggle(curves_value):
    ...
    '''
    Normalize the dispersion-curve plotting toggle to a single flag.
    '''
...
def resolve_dispersion_curve_config(config, reference_dir = None):

    '''
    Purpose
    -------
    Resolve the user dispersion-curve configuration against the pipeline defaults.
    ...
    reference_dir: str or pathlib.Path, optional
        Directory used to locate the legacy reference atmosphere and theory code.

    Outputs
    -------
    dispersion_config: dict
        Fully resolved dispersion-curve configuration.
    ...
    defaults = _build_default_dispersion_curve_config(reference_dir = reference_dir)
    user_config = {}

    if isinstance(config, dict):
        user_config = copy.deepcopy(config.get('dispersion_curves', {}))

    dispersion_config = _strip_dispersion_curve_style_keys(_deep_merge(defaults, user_config))
    dispersion_config['mode'] = _normalize_dispersion_curve_mode(dispersion_config.get('mode', 'simulation_based'))

    simulation_config = dispersion_config.setdefault('simulation_based', {})
    theory_config = dispersion_config.setdefault('theory_based', {})

    if simulation_config.get('reference_model_file', '') not in ['', None]:
        simulation_config['reference_model_file'] = str(
            Path(simulation_config['reference_model_file']).expanduser().resolve()
        )
    simulation_config['curves'] = _normalize_dispersion_curve_toggle(
        simulation_config.get('curves', {'enabled': True})
    )

    if not isinstance(theory_config.get('models', []), list):
        raise ValueError("dispersion_curves['theory_based']['models'] must be a list.")

    return dispersion_config


def resolve_legacy_reference_atmosphere_file(config = None, reference_dir = None):

    '''
    Purpose
    -------
    Resolve the reference atmosphere file used by the legacy overlay curves.
    ...
    reference_dir: str or pathlib.Path, optional
        Directory used to locate the legacy reference assets.

    Outputs
    -------
    model_file: pathlib.Path
        Resolved path to the reference atmosphere table.
    ...
    module_dir = _resolve_module_dir(reference_dir)
    dispersion_config = resolve_dispersion_curve_config(config or {}, reference_dir = module_dir)
    configured_model_file = dispersion_config.get('simulation_based', {}).get('reference_model_file', '')

    candidate_paths = []
    if configured_model_file not in ['', None]:
        candidate_paths.append(Path(configured_model_file).expanduser().resolve())

    candidate_paths.extend([
        module_dir / 'Oana_codes' / 'CSM_A.dat',
        module_dir / 'Oana Codes' / 'CSM_A.dat',
    ])
    ...
    raise FileNotFoundError(
        'Could not find the legacy reference atmosphere CSM_A.dat used by the simulation-based '
        'k-omega overlays.'
    )


def load_reference_atmosphere_model(config = None, reference_dir = None):

    '''
    Purpose
    -------
    Load the reference solar atmosphere used by the legacy k-omega overlays.
    ...
    -------
    atmosphere: dict
        Reference atmosphere columns used by the legacy dispersion boundaries.
    ...
    model_file = resolve_legacy_reference_atmosphere_file(config = config, reference_dir = reference_dir)
    model_data = np.loadtxt(model_file, dtype = np.float64)
    ...
    if model_data.ndim != 2 or model_data.shape[1] < 6:
        raise ValueError(
            f'Reference atmosphere file {model_file} must contain at least six columns.'
        )
    ...
    return {
        ...
        'density_cgs': np.asarray(model_data[:, 3], dtype = np.float64),
        'dRho_dz': np.asarray(model_data[:, 4], dtype = np.float64),
        'cs_cgs': np.asarray(model_data[:, 5], dtype = np.float64),
    }


def _build_dispersion_curve_record(name, k_axis, nu_axis):

    '''
    Purpose
    -------
    Build one standardized computed dispersion-curve record.
    ...
    k_axis: np.array
        Horizontal-wavenumber samples in 1/Mm.

    nu_axis: np.array
        Frequency samples in mHz.

    Outputs
    -------
    curve: dict
        Standardized dispersion-curve data record.
    ...
    return {
        ...
        'nu': np.asarray(nu_axis, dtype = np.float64),
    }


def compute_legacy_dispersion_curves(k_axis, config = None, reference_dir = None):

    '''
    Purpose
    -------
    Compute the legacy reference curves used by the original k-omega overlays.
    ...
    -------
    curves: list of dict
        Standardized computed curve records.
    ...
    dispersion_config = resolve_dispersion_curve_config(config or {}, reference_dir = reference_dir)
    simulation_config = dispersion_config.get('simulation_based', {})
    curve_specs = simulation_config.get('curves', {'enabled': True})
    atmosphere = load_reference_atmosphere_model(config = config, reference_dir = reference_dir)

    if not curve_specs.get('enabled', True):
        return []

    height_index = int(simulation_config.get('height_index', 1005))
    if height_index < 0 or height_index >= atmosphere['cs_cgs'].size:
        raise IndexError(
            f"dispersion_curves['simulation_based']['height_index'] = {height_index} "
            f'is out of range for the reference atmosphere.'
        )

    gravity_cgs = float(simulation_config.get('gravity_cgs', 27400.0))
    gravity_km = float(simulation_config.get('gravity_km_s2', 0.274))
    cs_cgs = np.asarray(atmosphere['cs_cgs'], dtype = np.float64)
    density_cgs = np.asarray(atmosphere['density_cgs'], dtype = np.float64)
    dRho_dz = np.asarray(atmosphere['dRho_dz'], dtype = np.float64)
    dP_dz = np.asarray(atmosphere['dP_dz'], dtype = np.float64)
    cs_km = cs_cgs*1.0e-5
    ...
    ordered_names = ['regime_1', 'regime_2', 'surface_fmode', 'lamb_curve']
    return [
        _build_dispersion_curve_record(
            curve_name,
            k_axis,
            curve_lookup[curve_name],
        )
        for curve_name in ordered_names
    ]


_THEORY_MODULE_CACHE = {}


def load_theory_modules(reference_dir = None):

    '''
    Purpose
    -------
    Load the theory configuration and solver modules used by theory-based overlays.

    Inputs
    ------
    reference_dir: str or pathlib.Path, optional
        Directory used to locate the `Code/Theory` module tree.

    Outputs
    -------
    modules: dict
        Dictionary containing the loaded theory config and solver modules.
    ...
    module_dir = _resolve_module_dir(reference_dir)
    theory_dir = module_dir.parent / 'Theory'
    cache_key = str(theory_dir)

    if cache_key in _THEORY_MODULE_CACHE:
        return _THEORY_MODULE_CACHE[cache_key]

    theory_config_spec = importlib.util.spec_from_file_location(
        f'time_distance_theory_config_{len(_THEORY_MODULE_CACHE)}',
        theory_dir / 'config.py',
    )
    theory_config_module = importlib.util.module_from_spec(theory_config_spec)
    theory_config_spec.loader.exec_module(theory_config_module)

    theory_functions_spec = importlib.util.spec_from_file_location(
        f'time_distance_theory_functions_{len(_THEORY_MODULE_CACHE)}',
        theory_dir / 'functions.py',
    )
    theory_functions_module = importlib.util.module_from_spec(theory_functions_spec)
    theory_functions_spec.loader.exec_module(theory_functions_module)

    _THEORY_MODULE_CACHE[cache_key] = {
        'config_module': theory_config_module,
        'functions_module': theory_functions_module,
...
def resolve_theory_model_params(model_name, model_spec, reference_dir = None):

    '''
    Purpose
    -------
    Resolve one theory-model parameter dictionary from the defaults plus user overrides.

    Inputs
    ------
    model_name: str
        Theory model identifier.

    model_spec: dict
        User model specification from the dispersion-curve config.

    reference_dir: str or pathlib.Path, optional
        Directory used to locate the `Code/Theory` modules.

    Outputs
    -------
    params: dict
        Fully resolved parameter dictionary for `DiagnosticDiagram`.
    ...
    theory_modules = load_theory_modules(reference_dir = reference_dir)
    theory_defaults = copy.deepcopy(theory_modules['config_module'].params)

    if model_name not in theory_defaults:
        raise KeyError(
            f'Unsupported theory dispersion model {model_name!r}. '
            "Choose from 'sf1966', 'mt1981', 'mt1982', 'bunte1993', or 'nc2009'."
        )

    default_params = copy.deepcopy(theory_defaults[model_name])
    params = copy.deepcopy(default_params)
    raw_overrides = copy.deepcopy(model_spec.get('params', {}))
    override_keys = set(raw_overrides.keys())

    if 'theta_deg' in raw_overrides and 'theta' not in raw_overrides:
        raw_overrides['theta'] = np.deg2rad(float(raw_overrides.pop('theta_deg')))
    if 'phi_deg' in raw_overrides and 'phi' not in raw_overrides:
        raw_overrides['phi'] = np.deg2rad(float(raw_overrides.pop('phi_deg')))
    if 'a_over_cs' in raw_overrides and 'a' not in raw_overrides:
        raw_overrides['a'] = float(raw_overrides['a_over_cs'])*float(params.get('cs', np.nan))

    params.update(raw_overrides)
    params['model'] = model_name

    # Recompute derived thermodynamic parameters when the caller changes the base inputs.
    cs_changed = (
        'cs' in params
        and 'cs' in default_params
        and not np.isclose(float(params['cs']), float(default_params['cs']))
    )
    g_changed = (
        'g' in params
        and 'g' in default_params
        and not np.isclose(float(params['g']), float(default_params['g']))
    )
    H_changed = (
        'H' in params
        and 'H' in default_params
        and not np.isclose(float(params['H']), float(default_params['H']))
    )

    if 'cs' in params and 'g' in params and 'H' in params:
        n_squared = (float(params['g'])/float(params['H'])) - (float(params['g'])**2/float(params['cs'])**2)
        derived_N = np.sqrt(max(n_squared, 0.0))
        if (cs_changed or g_changed or H_changed) and np.isclose(
            float(params.get('N', derived_N)),
            float(default_params.get('N', derived_N)),
        ):
            params['N'] = derived_N

    if 'cs' in params and 'H' in params:
        derived_wac = float(params['cs'])/(2.0*float(params['H']))
        if (cs_changed or H_changed) and np.isclose(
            float(params.get('wac', derived_wac)),
            float(default_params.get('wac', derived_wac)),
        ):
            params['wac'] = derived_wac

    # Recompute the Alfven-vector components when the user changes a/theta/phi.
    if model_name == 'nc2009':
        vector_keys = {'a', 'a_over_cs', 'theta', 'theta_deg', 'phi', 'phi_deg'}
        if len(vector_keys.intersection(override_keys)) > 0 and len({'ax', 'ay', 'az'}.intersection(override_keys)) == 0:
            a = float(params['a'])
            theta = float(params['theta'])
            phi = float(params['phi'])
            params['ax'] = a*np.sin(theta)*np.cos(phi)
            params['ay'] = a*np.sin(theta)*np.sin(phi)
            params['az'] = a*np.cos(theta)

    return params


def compute_theory_dispersion_curves(k_axis, nu_axis, config, reference_dir = None):

    '''
    Purpose
    -------
    Compute theory-based dispersion curves using the models in `Code/Theory`.
    ...
    if nu_axis is None or (isinstance(nu_axis, str) and str(nu_axis).strip() == ''):
        raise ValueError('theory_based dispersion curves require the plotted nu axis.')

    dispersion_config = resolve_dispersion_curve_config(config, reference_dir = reference_dir)
    theory_config = dispersion_config.get('theory_based', {})
    theory_modules = load_theory_modules(reference_dir = reference_dir)
    DiagnosticDiagram = theory_modules['functions_module'].DiagnosticDiagram

    k_axis = np.asarray(k_axis, dtype = np.float64)
    nu_axis = np.asarray(nu_axis, dtype = np.float64)
    omega_grid = np.asarray(nu_axis, dtype = np.float64)*1.0e-3*2.0*np.pi
    kh_grid = np.asarray(k_axis, dtype = np.float64)/1000.0
    curves = []
    fmode_added = False

    for model_index, model_spec in enumerate(theory_config.get('models', []), start = 1):
        if not model_spec.get('enabled', True):
            continue

        model_name = str(model_spec.get('model', '')).strip().lower()
        if model_name == '':
            raise ValueError("Each theory-based dispersion model must define a non-empty 'model' value.")

        params = resolve_theory_model_params(model_name, model_spec, reference_dir = reference_dir)
        diagram = DiagnosticDiagram(kh_grid, omega_grid, params)
        omega_roots = np.vstack([diagram.omega_solve(float(kh_value)) for kh_value in kh_grid])
        nu_roots = np.asarray(omega_roots, dtype = np.float64)/(2.0*np.pi)*1.0e3
        nu_roots = np.where(nu_roots > 0.0, nu_roots, np.nan)

        for branch_index in range(nu_roots.shape[1]):
            branch_values = np.asarray(nu_roots[:, branch_index], dtype = np.float64)
            if np.count_nonzero(np.isfinite(branch_values)) < 2:
                continue

            curves.append(
                _build_dispersion_curve_record(
                    f'{model_name}_branch_{branch_index + 1}',
                    k_axis,
                    branch_values,
                )
            )

        if theory_config.get('include_fmode', False) and not fmode_added:
            curves.append(
                _build_dispersion_curve_record(
                    'surface_fmode',
                    k_axis,
                    diagram.fmode_dispersion()/(2.0*np.pi)*1.0e3,
                )
            )
            fmode_added = True
...
def compute_dispersion_curves(config, k_axis, nu_axis = None, reference_dir = None):

    '''
    Purpose
    -------
    Compute the configured dispersion curves for k-omega and filter overlays.
    ...
    nu_axis: np.array, float, optional
        Frequency axis in mHz. Required for `theory_based` mode.

    reference_dir: str or pathlib.Path, optional
        Directory used to locate the legacy reference assets and `Code/Theory`.
    ...
    dispersion_config = resolve_dispersion_curve_config(config, reference_dir = reference_dir)
    mode = dispersion_config.get('mode', 'simulation_based')

    if mode == 'simulation_based':
        return compute_legacy_dispersion_curves(k_axis, config = config, reference_dir = reference_dir)

    return compute_theory_dispersion_curves(
        k_axis,
        nu_axis,
        config = config,
...
def parse_spectral_identifier(value):
    ...
    # Handle instrument-style bandpass labels that do not encode a chemical element token.
    instrument_line_patterns = [
        (r'(?:^|[^A-Za-z])(aia)[\s_.-]*(\d{3,5})(?:\s*(?:a|å|angstrom))?(?=[^0-9]|$)', 'AIA')]

    for candidate in candidates:
        for pattern, instrument in instrument_line_patterns:
            match = re.search(pattern, candidate, flags = re.IGNORECASE)
            if match is not None:
                spectral_id['element'] = instrument
...
def _parse_spectral_identifier_for_compact_token(value):

    '''
    Purpose
    -------
    Parse a spectral identifier using the legacy compact-token rules.

    This function preserves the original AIA wavelength extraction (e.g., 'aia1600')
    without applying the human-readable mapping. It is used solely for constructing
    the compact tokens required by phase-delay calibration metadata lookups.
    ...
    -------
    spectral_id: dict
        Dictionary with 'element' and 'line' suitable for compact-token formation.
    ...
    # Handle instrument-style bandpass labels using the original compact-token mapping.
    instrument_line_patterns = [
        (r'(?:^|[^A-Za-z])(aia)[\s_.-]*(\d{3,5})(?:\s*(?:a|å|angstrom))?(?=[^0-9]|$)', 'AIA')]
...
def normalize_paired_line_token(file_path):
    ...
    Notes
    -----
    This function uses the legacy compact-token parsing rules to ensure backward
    compatibility with existing phase-delay calibration entries. The compact tokens
    (e.g., 'aia1600') are preserved unchanged, while the display labels are handled
    separately by parse_spectral_identifier().
    ...
    spectral_id = _parse_spectral_identifier_for_compact_token(file_path)
...
def build_source_slug(config):
    ...
    if source_type == 'paired_cubes':
        ...
        if spectral_id_v1['element'] == '' or spectral_id_v2['element'] == '':
            raise ValueError(
...
def prepare_runtime_config(config):
    ...
    module_dir = runtime_config.get('_module_dir', Path(__file__).resolve().parent)
    data = runtime_config['data']
    paths = runtime_config.get('paths', {})
    source_type = _normalize_source_type(data['source_type'])
    data['source_type'] = source_type
    runtime_config.setdefault('time_distance', {})
    runtime_config['time_distance']['xcorr_geometry'] = normalize_xcorr_geometry(
        runtime_config['time_distance'].get('xcorr_geometry', _XCORR_GEOMETRY_DEFAULT))
    ...
    # Normalize the dispersion-curve config before numerical curve generation.
    runtime_config['dispersion_curves'] = resolve_dispersion_curve_config(
        runtime_config,
        reference_dir = module_dir,
    )
...
def normalize_xcorrj_engine(engine):

    '''
    Purpose
    -------
    Normalize the configured xcorrj execution engine.

    Inputs
    ------
    engine: object
        Raw engine label from the runtime configuration.

    Outputs
    -------
    normalized_engine: str
        Either `chunked` or `legacy`.
    ...
    if engine in ['', None]:
        engine = _XCORRJ_ENGINE_DEFAULT

    normalized_engine = str(engine).strip().lower()
    if normalized_engine not in ['chunked', 'legacy']:
        raise ValueError("time_distance['xcorrj_engine'] must be either 'chunked' or 'legacy'.")

    return normalized_engine


def normalize_bool_config(value, default):

    '''
    Purpose
    -------
    Normalize a permissive boolean configuration value.

    Inputs
    ------
    value: object
        Raw boolean-like configuration value.

    default: bool
        Value used when the raw input is blank.
    ...
    raise ValueError(f'Could not parse boolean configuration value {value!r}.')
...
def normalize_xcorr_geometry(geometry):
    ...
    Purpose
    -------
    Normalize the configured cross-correlation averaging geometry.

    Inputs
    ------
    geometry: object
        Raw geometry label from the runtime configuration.
    ...
    if geometry in ['', None]:
        geometry = _XCORR_GEOMETRY_DEFAULT

    normalized_geometry = str(geometry).strip().lower()
    if normalized_geometry not in _XCORR_GEOMETRIES:
        raise ValueError(
...
def get_xcorr_geometry(config):

    '''
    Purpose
    -------
    Resolve the active cross-correlation geometry from a runtime configuration.
    ...
    -------
    geometry: str
        Canonical cross-correlation geometry.
    ...
    return normalize_xcorr_geometry(
        config.get('time_distance', {}).get('xcorr_geometry', _XCORR_GEOMETRY_DEFAULT)
    )


def build_xcorr_geometry_slug(config_or_geometry):

    '''
    Purpose
    -------
    Return the filename slug for a cross-correlation geometry.

    Inputs
    ------
    config_or_geometry: dict or str
        Runtime configuration or raw geometry label.

    Outputs
    -------
    geometry_slug: str
        Empty for annulus compatibility, or the directional geometry name.
    ...
    if isinstance(config_or_geometry, dict):
        geometry = get_xcorr_geometry(config_or_geometry)
    else:
        geometry = normalize_xcorr_geometry(config_or_geometry)

    return '' if geometry == 'annulus' else geometry


def build_xcorr_geometry_title(config_or_geometry):

    '''
    Purpose
    -------
    Return the title fragment for a cross-correlation geometry.
    ...
    -------
    geometry_title: str
        Empty for annulus compatibility, or a human-readable wedge label.
    ...
    return f'{geometry.title()} Wedge'


def select_xcorr_offsets_for_geometry(yy_shift, xx_shift, geometry):

    '''
    Purpose
    -------
    Apply the configured annulus/wedge selection to one annular ring.

    Inputs
    ------
    yy_shift, xx_shift: np.array, int
        Offset vectors from the annulus center to each candidate annulus pixel.

    geometry: str
        Cross-correlation geometry label.

    Outputs
    -------
    selected_yy_shift, selected_xx_shift: np.array, int
        Offset vectors after applying the geometry mask.
    ...
    geometry = normalize_xcorr_geometry(geometry)
    yy_shift = np.asarray(yy_shift, dtype = np.int64)
    xx_shift = np.asarray(xx_shift, dtype = np.int64)

    if geometry == 'annulus':
        return yy_shift, xx_shift

    hypotenuse = np.hypot(xx_shift.astype(np.float64), yy_shift.astype(np.float64))
    valid = hypotenuse > 0.0
    cosine = np.divide(
        xx_shift,
        hypotenuse,
        out = np.zeros_like(hypotenuse, dtype = np.float64),
        where = valid)
    sine = np.divide(
        yy_shift,
        hypotenuse,
    ...
    if geometry == 'east':
        ...
        raise ValueError(f'Unsupported xcorr geometry: {geometry}')

    return yy_shift[mask], xx_shift[mask]


def resolve_xcorrj_chunk_centers(time_distance_config, nt):

    '''
    Purpose
    -------
    Resolve the number of annulus centers processed by one chunked xcorrj task.

    Inputs
    ------
    time_distance_config: dict
        Runtime time-distance configuration.

    nt: int
        Number of temporal samples in each center spectrum.
    ...
    requested_chunk_centers = time_distance_config.get(
        'xcorrj_chunk_centers',
        _XCORRJ_CHUNK_CENTERS_DEFAULT)

    if isinstance(requested_chunk_centers, str):
        requested_chunk_centers = requested_chunk_centers.strip().lower()

    if requested_chunk_centers not in ['', None, 'auto']:
        chunk_centers = int(requested_chunk_centers)
        if chunk_centers <= 0:
            raise ValueError("time_distance['xcorrj_chunk_centers'] must be positive or 'auto'.")
        return chunk_centers

    memory_mb = float(time_distance_config.get(
        'xcorrj_chunk_memory_mb',
        _XCORRJ_CHUNK_MEMORY_MB_DEFAULT))
    if memory_mb <= 0.0:
        raise ValueError("time_distance['xcorrj_chunk_memory_mb'] must be positive.")

    # One chunk keeps several [center, time] complex work arrays live at once.
    # Use a conservative byte estimate so concurrent workers do not accidentally
    # overcommit memory on full-size cubes.
    bytes_per_center = max(1, int(nt))*(6*16 + 2*8)
    memory_bytes = memory_mb*1024.0*1024.0
    chunk_centers = int(memory_bytes//float(bytes_per_center))

    return max(1, min(4096, chunk_centers))


def resolve_xcorrj_worker_count(config, cube_shape, num_tasks, engine, parallel_enabled):

    '''
    Purpose
    -------
    Resolve the effective xcorrj worker count for the selected execution engine.
    ...
    num_tasks: int
        Number of independent submitted work items.

    engine: str
        Selected xcorrj engine.
    ...
    capped: bool
        True when the configured worker count was capped.

    configured_workers: int
        Parsed requested worker count before capping.

    recommended_workers: int
        Engine-specific recommended worker count.
    ...
    num_tasks = max(1, int(num_tasks))
    if not parallel_enabled or num_tasks == 1:
        return 1, False, 1, 1

    cpu_count = max(1, os.cpu_count() or 1)
    requested_workers = config.get('time_distance', {}).get('nworkers', 'auto')

    if engine == 'legacy':
        recommended_workers = recommend_xcorrj_worker_count(config, cube_shape, num_tasks)
    else:
        recommended_workers = min(num_tasks, cpu_count)

    if isinstance(requested_workers, str):
        requested_workers = requested_workers.strip().lower()
    ...
    nworkers = min(num_tasks, configured_workers, recommended_workers)

    return nworkers, configured_workers > nworkers, configured_workers, recommended_workers


def prepare_xcorrj_geometry(ny, nx, deltas, width, dx_pixels, xcorr_geometry = _XCORR_GEOMETRY_DEFAULT):
    ...
    xcorr_geometry: str, optional
        Cross-correlation geometry used to select annulus points.
    ...
    xcorr_geometry = normalize_xcorr_geometry(xcorr_geometry)
    ...
    for delta in deltas:
        ...
        for offset in extent:
            ...
            yy_shift, xx_shift = select_xcorr_offsets_for_geometry(
                np.asarray(yy - center, dtype = np.int64),
                np.asarray(xx - center, dtype = np.int64),
                xcorr_geometry)

            per_ring_offsets.append({
                'yy_shift': yy_shift,
                'xx_shift': xx_shift,
                'count': int(yy_shift.size)})
...
def build_xcorrj_center_chunks(delta, chunk_centers):

    '''
    Purpose
    -------
    Split one radius' valid center pixels into deterministic chunk tasks.

    Inputs
    ------
    delta: int
        Radius, in pixels, for which to build chunks.

    chunk_centers: int
        Number of center pixels assigned to each chunk.

    Outputs
    -------
    chunks: list
        List of tuples `(delta, chunk_index, center_y, center_x)`.
    ...
    delta = int(delta)
    target_bounds = _target_bounds_by_delta[delta]
    chunk_centers = max(1, int(chunk_centers))

    y_values = np.arange(
        int(target_bounds['y_start']),
        int(target_bounds['y_stop']),
        dtype = np.int64)
    x_values = np.arange(
        int(target_bounds['x_start']),
        int(target_bounds['x_stop']),
        dtype = np.int64)

    # Preserve the legacy Fortran-order center traversal: y varies fastest and x
    # advances only after the full y column has been processed.
    center_y = np.tile(y_values, x_values.size)
    center_x = np.repeat(x_values, y_values.size)

    chunks = []
    for chunk_index, start_index in enumerate(range(0, center_y.size, chunk_centers)):
        stop_index = min(start_index + chunk_centers, center_y.size)
        chunks.append((
            delta,
            int(chunk_index),
            np.asarray(center_y[start_index:stop_index], dtype = np.int64),
            np.asarray(center_x[start_index:stop_index], dtype = np.int64)))

    return chunks


def chunked_parallel_loop(task):

    '''
    Purpose
    -------
    Compute one deterministic chunk of center pixels for one skip distance.

    Inputs
    ------
    task: tuple
        Tuple `(delta, chunk_index, center_y, center_x)` generated by
        build_xcorrj_center_chunks().

    Outputs
    -------
    result: tuple
        Tuple containing the radius, chunk index, center count, partial
        cross-correlation sum, partial phase sum, and elapsed seconds.
    ...
    delta, chunk_index, center_y, center_x = task
    delta = int(delta)
    chunk_index = int(chunk_index)
    center_y = np.asarray(center_y, dtype = np.int64)
    center_x = np.asarray(center_x, dtype = np.int64)
    center_count = int(center_y.size)
    _, _, nt = _fft_lower.shape

    if center_count <= 0:
        raise ValueError(f'Radius {delta}, chunk {chunk_index} produced an empty center set.')

    annulus_offsets = _annulus_offsets_by_delta[delta]
    phi1 = np.asarray(_fft_lower[center_y, center_x, :], dtype = np.complex128)
    phi2 = np.zeros((center_count, nt), dtype = np.complex128)

    for ring_offsets in annulus_offsets:
        ring_count = int(ring_offsets['count'])

        if ring_count <= 0:
            # Preserve the legacy/MATLAB behavior for empty wide-annulus bins:
            # a mean over an empty ring contributes NaNs to this radius.
            phi2 += np.full_like(phi2, np.nan + 0.0j)
            continue

        ring_sum = np.zeros_like(phi2)
        for yy_shift, xx_shift in zip(ring_offsets['yy_shift'], ring_offsets['xx_shift']):
            ring_sum += _fft_higher[center_y + int(yy_shift), center_x + int(xx_shift), :]

        phi2 += ring_sum/float(ring_count)

    phi2 /= float(_extent_size)

    xcorr_values = np.fft.ifft(np.conj(phi1)*phi2, axis = 1)
    xcorr_sum = np.asarray(xcorr_values.sum(axis = 0), dtype = np.complex128)

    phi1_phase = phi1 - phi1.mean(axis = 1, keepdims = True)
    phi2_phase = phi2 - phi2.mean(axis = 1, keepdims = True)
    phase_values = np.rad2deg(np.angle(
        np.fft.fftshift(phi1_phase*np.conj(phi2_phase), axes = 1)))
    phase_sum = np.asarray(phase_values.sum(axis = 0), dtype = np.float64)
...
def finalize_chunked_xcorrj_results(partial_results, deltas, nt):

    '''
    Purpose
    -------
    Reduce chunked xcorrj partial sums into final radius-ordered products.

    Inputs
    ------
    partial_results: list
        Partial chunk result tuples from chunked_parallel_loop().

    deltas: np.array
        Radius values that must appear in the output.
    ...
    -------
    results: list
        Radius-ordered tuples matching parallel_loop() output.

    diagnostics: dict
        Timing and chunk-count metadata for the benchmark harness.
    ...
    grouped_results = {int(delta): [] for delta in deltas}
    for partial_result in partial_results:
        grouped_results[int(partial_result[0])].append(partial_result)

    results = []
    chunk_timings = []
    per_radius = []

    for delta in [int(value) for value in deltas]:
        chunks = sorted(grouped_results[delta], key = lambda item: int(item[1]))
        if len(chunks) == 0:
            raise RuntimeError(f'No chunked xcorrj results were produced for radius {delta}.')
        ...
        for _, chunk_index, chunk_center_count, chunk_xcorr_sum, chunk_phase_sum, chunk_seconds in chunks:
            center_count += int(chunk_center_count)
            xcorr_sum += chunk_xcorr_sum
            phase_sum += chunk_phase_sum
            radius_seconds += float(chunk_seconds)
        ...
        per_radius.append({
            ...
            'sum_chunk_seconds': float(radius_seconds)})
...
class TimeDistance:
    ...
    def compute_single_cube_magnetic_orientation_metadata(self):
        ...
        with nc.Dataset(cube_file) as netcdf_file:
            ...
            reference_nz = None
            synthetic_zero_field_components = False
            synthetic_zero_field_variable = None
            synthetic_zero_field_axis_order = None
            ...
            try:
                ...
                if not loader.is_zero_field_simulation_path(cube_file):
                    raise

                synthetic_zero_field_components = True
                component_names = {
                ...
                component_variables = {}
                component_axis_orders = {}
                _, synthetic_zero_field_variable = loader.select_netcdf_observable_variable(netcdf_file)
                synthetic_zero_field_axis_order = loader.infer_netcdf_axis_order(synthetic_zero_field_variable)
                height_coordinates, current_height_coordinate_name = loader.load_netcdf_height_coordinates(netcdf_file, synthetic_zero_field_variable)
                nz = int(synthetic_zero_field_variable.shape[int(synthetic_zero_field_axis_order['z'])])
                resolved_height_indices = [
                    loader.resolve_netcdf_height_index(requested_height_indices[0], nz, 'h1'),
                    loader.resolve_netcdf_height_index(requested_height_indices[1], nz, 'h2')]
                selected_height_range = (
            ...
            for height_position, height_index in enumerate(resolved_height_indices):
                if synthetic_zero_field_components:
                    reference_slice = _extract_netcdf_height_slice(
                        synthetic_zero_field_variable,
                        synthetic_zero_field_axis_order,
                        height_index)
                    bx = np.zeros_like(reference_slice, dtype = np.float64)
                    by = np.zeros_like(reference_slice, dtype = np.float64)
                    bz = np.zeros_like(reference_slice, dtype = np.float64)
                else:
            ...
            for height_index in range(selected_height_range[0], selected_height_range[1] + 1):
                if synthetic_zero_field_components:
                    mean_field_strengths_G_between_heights.append(0.0)
                    continue
    ...
    def xcorrj(self, v1, v2):
        ...
        # Resolve the execution engine and worker policy. The chunked engine
        # parallelizes independent center chunks, while the legacy engine keeps
        # the original radius-level worker model for validation/debugging.
        engine = normalize_xcorrj_engine(self.time_distance.get('xcorrj_engine', _XCORRJ_ENGINE_DEFAULT))
        parallel_enabled = normalize_bool_config(
            self.time_distance.get('xcorrj_parallel', _XCORRJ_PARALLEL_DEFAULT),
            _XCORRJ_PARALLEL_DEFAULT)
        xcorrj_start_time = time.perf_counter()
        diagnostics = {
            'engine': engine,
            'parallel_enabled': bool(parallel_enabled),
            'xcorr_geometry': xcorr_geometry,
            'num_radii': int(deltas.size),
            'num_tasks': int(deltas.size),
            'chunk_centers': None,
            'chunk_memory_mb': float(self.time_distance.get(
                'xcorrj_chunk_memory_mb',
                _XCORRJ_CHUNK_MEMORY_MB_DEFAULT)),
            'configured_workers': None,
            'recommended_workers': None,
            'effective_workers': None,
            'worker_cap_applied': False,
            'chunk_timings': [],
            'per_radius': []}

        if engine == 'legacy':
            num_tasks = int(deltas.size)
            ncores, capped, configured_workers, recommended_workers = resolve_xcorrj_worker_count(
                self.config,
                (nt, ny0, nx0),
                num_tasks,
                engine,
                parallel_enabled)
            diagnostics['configured_workers'] = int(configured_workers)
            diagnostics['recommended_workers'] = int(recommended_workers)
            diagnostics['effective_workers'] = int(ncores)
            diagnostics['worker_cap_applied'] = bool(capped)
            if capped:
                print(
                    f'Configured nworkers = {configured_workers}; '
                    f'capping to {recommended_workers} for this workload.')
            print(f'Running xcorrj legacy engine across {ncores} worker threads')
            ...
            chunk_centers = resolve_xcorrj_chunk_centers(self.time_distance, nt)
            chunk_tasks = []
            for delta in deltas:
                chunk_tasks.extend(build_xcorrj_center_chunks(int(delta), chunk_centers))

            num_tasks = int(len(chunk_tasks))
            ncores, capped, configured_workers, recommended_workers = resolve_xcorrj_worker_count(
                self.config,
                (nt, ny0, nx0),
                num_tasks,
                engine,
                parallel_enabled)
            diagnostics['num_tasks'] = int(num_tasks)
            diagnostics['chunk_centers'] = int(chunk_centers)
            diagnostics['configured_workers'] = int(configured_workers)
            diagnostics['recommended_workers'] = int(recommended_workers)
            diagnostics['effective_workers'] = int(ncores)
            diagnostics['worker_cap_applied'] = bool(capped)
            if capped:
                print(
                    ...
                    f'capping to {recommended_workers} for this workload.')
            print(
                f'Running xcorrj chunked engine across {ncores} worker threads '
                f'({num_tasks} chunks, {chunk_centers} centers/chunk)')

            partial_results = []
            if ncores == 1:
                for task in tqdm(chunk_tasks, desc = 'Computing skip-distance chunks', unit = 'chunk'):
                    delta = int(task[0])
                    chunk_index = int(task[1])
                    try:
                        partial_results.append(chunked_parallel_loop(task))
                    except Exception as exc:
                        raise RuntimeError(
                            f'Radius {delta}, chunk {chunk_index} failed during xcorrj.') from exc
            else:
                with ThreadPoolExecutor(max_workers = ncores) as executor:
                    future_to_task = {executor.submit(chunked_parallel_loop, task): task for task in chunk_tasks}

                    with tqdm(total = len(chunk_tasks), desc = 'Computing skip-distance chunks', unit = 'chunk') as pbar:
            ...
            results, chunked_diagnostics = finalize_chunked_xcorrj_results(
                partial_results,
                deltas,
                nt)
            diagnostics['chunk_timings'] = chunked_diagnostics['chunk_timings']
            diagnostics['per_radius'] = chunked_diagnostics['per_radius']

        diagnostics['wall_seconds'] = float(time.perf_counter() - xcorrj_start_time)
        self.xcorrj_diagnostics = diagnostics
    ...
    def save_time_distance(
        ...
        core_header['XCGEO'] = (xcorr_geometry, 'Cross-correlation averaging geometry')
        core_header['SRCMODE'] = str(self.data.get('source_type', ''))
        fits.PrimaryHDU(np.asarray(xc, dtype = np.float32), header = core_header).writeto(
            self.outfile,
            overwrite = True)
        phase_header = core_header.copy()
        phase_header['BUNIT'] = ('deg', 'Phase-difference units')
        fits.PrimaryHDU(np.asarray(phase_diff, dtype = np.float32), header = phase_header).writeto(
            self.phase_outfile,
            overwrite = True)
        ...
        if komega_spectrum is not None:
            ...
            k_header['XCGEO'] = (xcorr_geometry, 'Run cross-correlation geometry')
        ...
        if coherence_spectrum is not None:
            ...
            c_header['XCGEO'] = (xcorr_geometry, 'Run cross-correlation geometry')
    ...
    def run(self):
        ...
        self.results = {
            ...
            'xcorr_geometry': get_xcorr_geometry(self.config),
            'magnetic_orientation': copy.deepcopy(self.magnetic_orientation_metadata),
            'xcorrj_diagnostics': copy.deepcopy(self.xcorrj_diagnostics),
...
# Populate this module-level reference when the pipeline imports `filter.py`.
AGWFilter = None
...
def load_agw_filter_class(config_file):
    ...
    ------
    config_file: str or pathlib.Path
        Configuration file used to locate `filter.py`.
    ...
    # Resolve the sibling `filter.py` path next to the active config.
    agw_filter_file = config_file.parent / 'filter.py'
